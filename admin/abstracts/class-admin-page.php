<?php
/**
 * Pretty_Opt_In_Admin_Page Class
 *
 * @since  1.0.0
...
if ( ! class_exists( 'Pretty_Opt_In_Admin_Page' ) ) :
    
    abstract class Pretty_Opt_In_Admin_Page {
	...
	public function render_page_hooks() {
        ...
        ?>
            <main class="pretty-wrap <?php echo esc_attr( 'pretty-opt-in-' . $this->page_slug ); ?>">

                <?php
                $this->render_menu();
                ?>

                <div class="pretty-content-wrap">
        ...
        protected function render_footer() {
            ...
            $footer_text = sprintf(/* translators: ... */
                esc_html__( 'Made with %s by WPHobby', 'pretty-opt-in' ),
                ' <ion-icon class="pretty-icon-heart" name="heart"></ion-icon>'
            );
            ...
            ?>
            <div class="pretty-footer"><?php echo $footer_text; ?></div>
        ...
        public function template( $path, $args = array(), $echo = true ) {
            $file    = PRETTY_OPT_IN_DIR . "/admin/views/$path.php";
            $content = '';
        ...
        public function init_scripts( $hook ) {
            // Init jquery ui
            pretty_opt_in_admin_jquery_ui_init();
        }
